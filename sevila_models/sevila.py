...
class SeViLA(Blip2Base):
    ...
    def __init__( self, img_size=224, drop_path_rate=0,
        ...
        if 'freeze_loc' in task:
            ...
            for name, param in self.t5_proj_loc.named_parameters():
                param.requires_grad = False
    ...
    def forward(self, samples,
        ...
        image_embeds = self.visual_encoder(image) #32 257 1408
        ...
        if 'train_loc' in self.task:
            ...
            with torch.no_grad():
                ...
                ans_idx = [self.ANS_MAP[a[7]] for a in answer]
        ...
        # Finetune answerer with localizer
            ...
            # # 初始化结果列表
            # select_frames_idx = []
            # candidate_frames_idx = torch.topk(loc_yes, 8, dim=-1).indices.tolist()

            # # 遍历每一行
            # for row,add in zip(loc_yes, candidate_frames_idx):
            #     # 拆分为每 4 个元素一组
            #     groups = [row[i:i+group_size] for i in range(0, len(row), group_size)]
                
            #     # 对每组应用 topk 函数获取最大值的索引
            #     max_indices = [torch.topk(group, 1, dim=0).indices.item() + i for i, group in enumerate(groups)]
            #     while len(max_indices) < target_length:
            #         for candidate in add:
            #             # 如果候选元素不在现有列表中，则添加
            #             if candidate not in max_indices:
            #                 max_indices.append(candidate)
            #                 # 达到目标长度后退出循环
            #                 if len(max_indices) == target_length:
            #                     break
            #                     # 将结果添加到结果列表中
            #     select_frames_idx.append(max_indices)
